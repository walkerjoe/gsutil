...
NON_EXISTENT_OBJECT_REGEX = re.compile(
    r'.*non-\s*existent\s*object', flags=re.DOTALL)
...
class BotoTranslation(CloudApi):
  ...
  def CreateBucket(self,
    ...
    if bucket_uri.scheme == 'gs':
      ...
      if (metadata and metadata.retentionPolicy
  ...
  def DeleteBucket(self, bucket_name, preconditions=None, provider=None):
    ...
    try:
      ...
      if (translated_exception
          and 'BucketNotEmpty' in translated_exception.reason):
        try:
    ...
    try:
      for key in objects_iter:
        if yield_prefixes and isinstance(key, Prefix):
          ...
          if not get_fields or (get_fields
                                and not get_fields.issubset(key_http_fields)):
      ...
      if digesters:
        ...
        for alg_name in digesters:
          if ((download_strategy == CloudApi.DownloadStrategy.RESUMABLE
               and start_byte != 0)
              or not ((getattr(key, 'local_hashes', None)
  ...
  def UploadObjectResumable(self,
    ...
    try:
      self._PerformResumableUpload(
          ...
          progress_callback=progress_callback,
          headers=headers)
      return self._HandleSuccessfulUpload(
          dst_uri, object_metadata, fields=fields)
    except TRANSLATABLE_BOTO_EXCEPTIONS as e:
  ...
  def ComposeObject(self,
    ...
    try:
      ...
      return self.GetObjectMetadata(
          dst_bucket_name, dst_obj_name, fields=fields)
    except TRANSLATABLE_BOTO_EXCEPTIONS as e:
  ...
  def _BotoBucketToBucket(self, bucket, fields=None):
    ...
    cloud_api_bucket = apitools_messages.Bucket(
        name=bucket.name, id=bucket.name)
    headers = self._CreateBaseHeaders()
    if self.provider == 'gs':
      ...
      if not fields or 'acl' in fields:
        try:
          ...
          if (translated_exception
              and isinstance(translated_exception, AccessDeniedException)):
    ...
    if not fields or 'versioning' in fields:
      ...
      if versioning:
        if (self.provider == 's3' and 'Versioning' in versioning
  ...
  def _BotoKeyToObject(self, key, fields=None):
    ...
    if not fields or 'acl' in fields:
      ...
      self._TranslateBotoKeyAcl(
          key, cloud_api_object, generation=generation_str)
